...
import requests
import time
source = requests.get('https://www.cricbuzz.com/cricket-schedule/upcoming-series/women').text
soup = BeautifulSoup(source, 'lxml')

def womens_fixtures():
    main_div = soup.find('div', class_ = 'cb-bg-white cb-schdl cb-col cb-col-100').find('div', id = 'women-list')
    womens_fixtures = []
    for content_div in main_div.find_all('div', class_ = 'cb-col-100 cb-col'):
        ...
        for match_details, location in zip(matches.find_all('div', class_ = 'cb-ovr-flo cb-col-60 cb-col cb-mtchs-dy-vnu cb-adjst-lst'), matches.find_all('div', class_ = 'cb-col-40 cb-col cb-mtchs-dy-tm cb-adjst-lst')):
            ...
            if temp not in womens_fixtures:
                womens_fixtures.append(temp)
    #for fixtures in womens_fixtures:
    #    print(fixtures)
    return womens_fixtures
