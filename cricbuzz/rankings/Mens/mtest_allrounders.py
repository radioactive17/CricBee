...
def mtest_allrounders():
    source = requests.get('https://www.icc-cricket.com/rankings/mens/player-rankings/test/all-rounder').text
    soup = BeautifulSoup(source, 'lxml')
    main_div = soup.find('div', class_ = 'rankings-block__container full')
    mtest_allrounders = [[] for _ in range(20)]
    ...
    mtest_allrounders[0].extend([first_rank, player, team, rating])
    i = 1
    for other in main_div.find_all('tr', class_ = 'table-body'):
        ...
        mtest_allrounders[i].extend([rank, name, team, rating])
        i += 1

    for allrounders in mtest_allrounders:
        print(allrounders)
...
if __name__ == '__main__':
    while True:
        mtest_allrounders()
        wait_time = 1
        print(f'\nWaiting {wait_time} minute before fetching Mens Test Allrounders Rankings again...\n')
