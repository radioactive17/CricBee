...
def mt20_batting():
    source = requests.get('https://www.icc-cricket.com/rankings/mens/player-rankings/t20i/batting').text
    soup = BeautifulSoup(source, 'lxml')
    main_div = soup.find('div', class_ = 'rankings-block__container full')
    mt20_batting = [[] for _ in range(100)]
    ...
    mt20_batting[0].extend([first_rank, player, team, rating])
    i = 1
    for other in main_div.find_all('tr', class_ = 'table-body'):
        ...
        mt20_batting[i].extend([rank, name, team, rating])
        i += 1

    for batters in mt20_batting:
        print(batters)
    #return mt20_batting
...
if __name__ == '__main__':
    while True:
        mt20_batting()
        wait_time = 1
        print(f'\nWaiting {wait_time} minute before fetching Mens ODI Batting Rankings again...\n')
