...
def odi_team():
    source = requests.get('https://www.icc-cricket.com/rankings/womens/team-rankings/odi').text
    soup = BeautifulSoup(source, 'lxml')
    odi_team_rankings = [[] for _ in range(11)]
    ...
    odi_team_rankings[0].extend([f_rank, f_team, f_matches, f_points, f_rating])
    i = 1
    for other in main_div.find_all('tr', class_ = 'table-body'):
        ...
        odi_team_rankings[i].extend([rank, team, matches, points, rating])
        i += 1

    for rank in odi_team_rankings:
        print(rank)
    # return odi_team_rankings
...
if __name__ == '__main__':
    while True:
        odi_team()
        wait_time = 1
        print(f'\nWaiting {wait_time} minute before fetching Womens ODI Rankings again...\n')
